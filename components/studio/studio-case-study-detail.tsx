    ...
    <div className="relative isolate overflow-visible rounded-[1.85rem]">
        ...
        <div className="relative z-10 border-b border-[var(--color-border-default)]/80 pb-8">
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(18rem,0.8fr)] lg:items-start">
            <div>
              ...
              <h1
                className={cn(
                  "mt-2 max-w-2xl font-display leading-[0.92] tracking-[-0.05em] text-[var(--neutral-950)]",
                  isModal
      ...
      <div className="relative z-10 grid gap-4 border-b border-[var(--color-border-default)]/80 py-8 lg:grid-cols-[minmax(0,1.05fr)_minmax(18rem,0.95fr)]">
        <PremiumSurface
          ...
          blur="lg"
          radius="xl"
          className="self-start min-h-[18rem] overflow-visible border-white/42 bg-[linear-gradient(180deg,rgba(255,255,255,0.36),rgba(255,255,255,0.18))] p-5 shadow-[0_12px_28px_rgba(15,23,42,0.04)] sm:min-h-[24rem] sm:p-6 lg:sticky lg:top-6"
        >
            ...
            <div className="space-y-6">
              {detail.sections.map((section, index) => (
                <section key={section.title} className="pt-1">
                  <p className="text-label-sm uppercase tracking-[0.16em] text-[var(--color-text-brand)]">
                    {String(index + 1).padStart(2, "0")}
                  </p>
                  <h2 className="mt-2 text-heading-sm text-[var(--neutral-950)]">
                    {section.title}
                  </h2>
                  <p className="mt-2 text-body-sm leading-7 text-[var(--color-text-secondary)]">
                    {section.body}
                  </p>
                </section>
              ))}
        ...
        {detail.proofPoints.map((point) => {
          ...
          return (
            <section key={point.title} className="space-y-4 pt-1">
              ...
              <div>
                <h3 className="text-body-lg font-semibold text-[var(--neutral-950)]">
                  {point.title}
                </h3>
                <p className="mt-3 max-w-sm text-body-sm leading-7 text-[var(--color-text-secondary)]">
                  {point.description}
                </p>
              </div>
            </section>
