...
## Motion & Interaction Layer (Extended)

### Objective
Introduce consistent, high-quality motion and interaction behavior across the UI using Framer Motion, GSAP, and Three.js without breaking the design system.
...
- Use Framer Motion by default unless GSAP is clearly required
- Three.js must remain isolated from DOM UI layers
- Animation must not break layout or responsiveness

## Micro-Interactions (Framer Motion)

### Objective
Enhance UX through subtle, fast, and meaningful interactions.
...
- Use `ease-out` as default easing
- Avoid bounce unless explicitly needed
...
Use shared motion variants instead of inline animations.


## GSAP (Scroll & Timeline Animations)

### Objective
Handle complex motion systems beyond Framer Motion capabilities.
...
- Always use `ScrollTrigger` for scroll-based animations
- Keep animations smooth and GPU-friendly
- Avoid excessive motion that distracts from content
- Do not use GSAP for simple hover or tap interactions
...
### Objective
Implement immersive 3D experiences without breaking UI structure.
...
- Three.js must be rendered in a separate canvas layer
- UI (React components) must stay independent of 3D scene
- Use `react-three-fiber` when possible
- Use `drei` helpers for faster setup

---

## Interaction Rules

- Camera movement should be smooth and controlled
- Avoid abrupt transitions
- Sync scroll position with camera only when necessary
...
- 3D should enhance, not dominate
- Always maintain readability of text over 3D

## Motion + Design System Integration

### Objective
Ensure motion works WITH the design system, not against it.

---

## Rules

- Motion must respect spacing and layout tokens
- Animations must not introduce visual inconsistency
...
- Hero CTA → stronger motion
- Secondary buttons → subtle motion
- Background elements → minimal motion
...
- Avoid heavy scroll animations on low-end devices
- Ensure touch interactions feel instant
...
- Maintain performance over visual effects

## Motion Validation (Add to Validation Section)

- Animations do not break layout
- No lag or frame drops
- Works smoothly on mobile
- No conflicting animation libraries on same element
- 3D does not block UI interaction

## Motion Guardrails

- Do not add animation without purpose
- Do not overuse GSAP for simple interactions
- Do not couple Three.js logic with UI components
- Do not create inconsistent motion styles across components